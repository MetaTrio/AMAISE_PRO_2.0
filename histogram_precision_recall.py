...
# #     # Load the sequences_over_1000_ids_with_len.txt (no need to filter)
# #     length_data = pd.read_csv(length_file, sep=",", header=None, names=["id", "len"])

# #     # Load the trueAndPredictedLabels.csv (no length column here)
# #     predictions_data = pd.read_csv(predictions_file, header=None, names=["id", "y_true", "pred_label"])

# #     # Filter predictions to include only reads present in length_data (those with length > 1000)
# #     filtered_data = predictions_data.merge(length_data, on="id")

# #     # Define length bins
# #     bin_edges = np.arange(1000, filtered_data["len"].max() + 500, 500)  # 500bp bins
...
#     # Load the trueAndPredictedLabels.csv (no length column here), skip the first row if it has headers
#     # predictions_data = pd.read_csv(predictions_file, header=None, names=["id", "y_true", "pred_label"], skiprows=1, dtype={"id": str, "y_true": int, "pred_label": int})
#     # Load the CSV, attempt to coerce to int for 'pred_label'
#     predictions_data = pd.read_csv(predictions_file, header=0, dtype={"id": str, "y_true": int, "pred_label": float})

#     # If 'pred_label' contains unexpected values, you can convert them to integers or handle them as needed:
...
#     # Optionally adjust figure size if necessary
...
# File paths
length_file_path = "/home/cseroot/sequences_over_1000_ids_with_len.txt"
predictions_file_path = "/home/cseroot/tishani.20/amaise_train_pacbio/test/SRR11547004_thr65/trueAndPredictedLabels.csv"
output_file_path = "/home/cseroot/tishani.20/amaise_train_pacbio/test/SRR11547004_thr65/precision_recall_histogram.png"
